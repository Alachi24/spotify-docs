...
- **Token Lifespan**: [Access tokens](/docs/Documentation/Authentication/Access_Token) are designed with a limited lifespan of 1 hour. Once expired, new access tokens can be obtained by providing the original refresh token, which is issued during the authorization token request response.
